        ...
        <div style="margin-top:100px; position: absolute; width: 100%" class="tampil">
          <div id="v_banten" style="width: 80%">
              <div class="pembatas" style="width: 90%; height: 50px; background-color: #dcdde1; border-radius: 7px">
                <h1 style="text-align: center; color: #222f3e;">SEMUA BANTEN</h1>
              </div><br>
              <?php 
                include '../connect.php';

                $qbanten = 'SELECT sarana.sarana_id, fot_sar.url_sarana, sarana.sarkat_id, user.user_id, sarana.sar_nama, sarana.des_sar, sarana.dibuat, sarana.stock_sar, sarana.sar_price, sarana.pilihan, sar_kategori.sarkat_nama, user.username FROM sarana, sar_kategori, fot_sar, user WHERE sarana.sarkat_id = sar_kategori.sarkat_id AND sarana.user_id = user.user_id AND sarana.sarana_id = fot_sar.sarana_id';
                
                $resba = $conn->query($qbanten);
                $total = mysqli_num_rows($resba);
                if ($total > 0) {
                  while ($rowba = $resba -> fetch_assoc()) {
                    echo "<a style='align-item: center' href='../banten/belibanten.php?data_id=".$rowba['sarana_id']."' id='custId' data-toggle='modal'>";
                    ...
                    echo "<h3 class='card-title' style='color: black; text-align: center; margin-top: -5px;'>".$rowba['sar_nama']."</h3>";
